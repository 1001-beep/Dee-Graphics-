      ...
      {attempted && lookup && (
        <div style={{ ...glass, padding: 36, borderRadius: 2 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28, paddingBottom: 20, borderBottom: `1px solid ${C.border}` }}>
            ...
            <span
              style={{
                ...
                textTransform: "uppercase",
                color: statusColor(lookup.orderState),
                border: `1px solid ${statusColor(lookup.orderState)}33`,
                padding: "6px 12px",
                borderRadius: 2,
                background: statusColor(lookup.orderState) + "11",
              }}
            >
              {lookup.orderState}
            </span>
          ...
          {[
            ...
            [
              "Order Placed",
              new Date(lookup.timestampISO).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }),
            ],
          ].map(([label, value]) => (
          ...
          <p style={{ fontSize: 11, color: C.gray600, marginTop: 20, textAlign: "center" }}>
            Questions? Reply to your confirmation email or use the order form.
          </p>
        </div>
      )}
