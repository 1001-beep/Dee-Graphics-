...
export default function RequestPage() {
  ...
  const realSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send email (non-critical — won't block success)
      try {
        await emailjs.send(
          EMAILJS_SERVICE_ID,
          EMAILJS_TEMPLATE_ID,
        ...
        {state.submitted ? (
          <div style={{ textAlign: "center", padding: "24px 0" }}>
            ...
            <div style={{ ...glass, padding: 24, borderRadius: 2, border: `1px solid rgba(37,211,102,0.25)`, background: "rgba(37,211,102,0.05)", marginBottom: 24 }}>
              ...
              <a href={WHATSAPP_LINK} target="_blank" rel="noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "#25D366", color: "#07070A", fontWeight: 700, fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", padding: "12px 28px", borderRadius: 2, textDecoration: "none", boxShadow: "0 8px 24px rgba(37,211,102,0.2)" }}>
                <Icon d={WHATSAPP_ICON} size={16} />
                Track on WhatsApp
              </a>
            </div>
            <button onClick={() => setState({ submitted: false, processing: false, trackingId: "", error: "" })} style={{ background: "none", border: `1px solid ${C.border}`, color: C.gray400, fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", padding: "10px 24px", cursor: "pointer", borderRadius: 2 }}>
              Submit Another
            </button>
          </div>
        ) : (
          <form onSubmit={realSubmit} style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div>
                ...
                <input style={inputStyle} required placeholder="e.g. Alex Morgan" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} onFocus={(e) => (e.target.style.borderColor = C.gold)} onBlur={(e) => (e.target.style.borderColor = C.border)} />
              </div>
              <div>
                <label style={labelStyle}>Email Address</label>
                <input type="email" style={inputStyle} required placeholder="you@company.com" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} onFocus={(e) => (e.target.style.borderColor = C.gold)} onBlur={(e) => (e.target.style.borderColor = C.border)} />
              </div>
            </div>
            <div>
              <label style={labelStyle}>Project Type</label>
              <select style={{ ...inputStyle, appearance: "none", cursor: "pointer" }} value={form.projectType} onChange={(e) => setForm((f) => ({ ...f, projectType: e.target.value }))}>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                {[["Standard", "5–7 days"], ["Priority", "48 hours"], ["Rush", "24 hours"]].map(([speed, note]) => (
                  <button type="button" key={speed} onClick={() => setForm((f) => ({ ...f, speed }))} style={{ padding: "12px 8px", border: `1px solid ${form.speed === speed ? C.gold : C.border}`, background: form.speed === speed ? `rgba(212,175,55,0.08)` : C.surfaceAlt, color: form.speed === speed ? C.gold : C.gray400, fontSize: 11, cursor: "pointer", borderRadius: 2, transition: "all .2s" }}>
            ...
            <div>
              ...
              <textarea style={{ ...inputStyle, minHeight: 120, resize: "vertical" }} required placeholder="Describe your project — brand name, target audience, style references, deliverables needed..." value={form.scopeDoc} onChange={(e) => setForm((f) => ({ ...f, scopeDoc: e.target.value }))} onFocus={(e) => (e.target.style.borderColor = C.gold)} onBlur={(e) => (e.target.style.borderColor = C.border)} />
            </div>
