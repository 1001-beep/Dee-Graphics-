...
function SelectWithArrow({ value, onChange, options, style }: {
  value: string;
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      >
        {options.map((o) => (
          <option key={o} value={o} style={{ background: "#0F0F15" }}>{o}</option>
        ))}
      </select>
      <div style={{
...
export default function RequestPage() {
  const [form, setForm] = useState({
    ...
    subOption: SUB_OPTIONS["Visual Identity"][0],
    speed: "Standard",
    scopeDoc: "",
    name: "",
    email: "",
    ...
    try {
      await addDoc(collection(db, "designRequests"), {
        ...
        await emailjs.send(EMAILJS_SERVICE_ID, CLIENT_TEMPLATE_ID, {
          ...
          speed: form.speed, tracking_id: trackingId, price, estimated_delivery: estimatedDelivery,
        }, EMAILJS_PUBLIC_KEY);
      } catch (emailErr) {
      ...
      <div style={{ ...glass, padding: 40, borderRadius: 2 }}>
        {state.submitted ? (
          <div style={{ textAlign: "center", padding: "24px 0" }}>
            ...
            <h3 style={{ fontSize: 20, fontWeight: 500, color: C.white, marginBottom: 8 }}>🎉 Congratulations, {form.name || "there"}!</h3>
            ...
            <div style={{ ...glass, padding: "16px 24px", borderRadius: 2, marginBottom: 20, border: `1px solid rgba(212,175,55,0.2)` }}>
              ...
              <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.border}`, textAlign: "left" }}>
                ...
                <code style={{ fontSize: 14, color: C.gold, fontFamily: "monospace" }}>{state.trackingId}</code>
              </div>
              <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.border}`, textAlign: "left" }}>
            ...
            <p style={{ color: C.gray600, fontSize: 11, marginBottom: 24, letterSpacing: "0.05em" }}>Save your Tracking ID to check project status in the Client Portal.</p>
            <div style={{ ...glass, padding: 24, borderRadius: 2, border: `1px solid rgba(37,211,102,0.25)`, background: "rgba(37,211,102,0.05)", marginBottom: 24 }}>
              <p style={{ color: C.gray400, fontSize: 13, marginBottom: 16, lineHeight: 1.7 }}>
                Tap below to send your full order details to us on WhatsApp — your tracking ID, service, and price included automatically.
              </p>
            ...
            {!isBespoke && (
              <div>
                ...
                <SelectWithArrow
                  ...
                  onChange={(v) => setForm((f) => ({ ...f, subOption: v }))}
                  options={SUB_OPTIONS[form.projectType] || []}
                />
              </div>
            )}
            ...
            <div>
              <label style={labelStyle}>
                {isBespoke ? "Describe Your Project" : "Additional Notes (optional)"}
              </label>
              <textarea
                style={{ ...inputStyle, minHeight: 120, resize: "vertical" }}
                required={isBespoke}
                placeholder={isBespoke
                  ? "Describe your custom project — what you need, style references, deliverables..."
                  : "Any specific style, colors, references or details you'd like us to know..."}
                value={form.scopeDoc}
